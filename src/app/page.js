...
export default function Homepage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to fetch products, please try again later. ${res.statusText}`,
          );
        }
  ...
  return (
    <>
      {loading && (
        <p className="text-purple mt-10 animate-pulse text-center text-4xl">
          Loading...
        </p>
      )}

      {error && !loading && (
        <p className="mt-20 mb-4 text-center text-4xl text-red-500">{error}</p>
      )}

      {!loading && !error && (
        <main
          className={`${loading ? "hidden" : "flex"} w-full max-w-7xl flex-col items-center gap-4 py-4 lg:flex-row-reverse lg:items-start lg:gap-6`}
        >
          {/* list category */}
